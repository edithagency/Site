...
export default function ClientCard({ client, style }: ClientCardProps) {
  return (
    <Link href={`/projets/${client.slug}`} className="group block" style={style}>
      <div className="relative overflow-hidden rounded-lg bg-brand-deep mb-4" style={{ aspectRatio: '3/4' }}>
        ...
        {client.cardImage ? (
          <Image
            ...
            alt={client.name}
            fill
            sizes="(max-width: 768px) 34vw, 21vw"
      ...
      <p className="font-display font-bold text-brand-deep text-xl group-hover:text-brand-mid transition-colors">
        {client.name}
      </p>
    </Link>
  )
